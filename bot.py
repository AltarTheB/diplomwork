...
from pars import KolesaKz, get_marks, get_regions, get_models, bodywork, KKP, customs_clearance, state

from time import sleep
from multiprocessing import Process

bot = TeleBot('1373577825:AAFnNlVipbLHFYVRMsr3ScoEB_NMULejBa4')

@bot.message_handler(commands= ['start'])
def main(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    item1 = types.KeyboardButton('/подпись')
...
@bot.message_handler(commands=['подпись'])
...
def handler_callback_query(query):
	if query.data.startswith('1.'):
	...
	elif query.data.startswith('22.'):
		keyboard = types.InlineKeyboardMarkup()

		for i in customs_clearance.keys():
			keyboard.add(types.InlineKeyboardButton(i, callback_data='22.'+customs_clearance[i]+'.'+query.data.split('22.')[1]))
		keyboard.add(types.InlineKeyboardButton('Неважно', callback_data='22.0.'+query.data.split('22.')[1]))

		bot.edit_message_text(chat_id=query.from_user.id, message_id=query.message.message_id, text="Растаможен:", reply_markup=keyboard)
	elif query.data.startswith('23.'):
		keyboard = types.InlineKeyboardMarkup()

		for i in state.keys():
			keyboard.add(types.InlineKeyboardButton(i, callback_data='23.'+state[i]+'.'+query.data.split('23.')[1]))
		keyboard.add(types.InlineKeyboardButton('Неважно', callback_data='23.0.'+query.data.split('23.')[1]))

		bot.edit_message_text(chat_id=query.from_user.id, message_id=query.message.message_id, text="Состояние:", reply_markup=keyboard)
		...
		db.close()
	elif query.data.startswith('14.') or query.data.startswith('15.') or query.data.startswith('16.') or query.data.startswith('17.') or query.data.startswith('18.') or query.data.startswith('22.') or query.data.startswith('23.'):
		...
		filters_keyboard.row(types.InlineKeyboardButton('Разтаможен', callback_data='22.'+data), types.InlineKeyboardButton('Состояние', callback_data='23.'+data),types.InlineKeyboardButton('Сброс', callback_data='13.'+data))
		...
		if filters['state'] != None:
			text += 'Состояние: {}\n'.format(filters['state'])
		else:
			text += 'Состояние: не важен\n'
		
		if filters['customs-clearance'] != None:
			text += 'Разтаможка: {}\n'.format(filters['customs-clearance'])
		else:
			text += 'Разтаможка: не важен\n'
...
if __name__==  '__main__':
	Process(target=schedule, args=(30, )).start()
